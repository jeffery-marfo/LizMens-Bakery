...
const Order = () => {
  ...
  const [paymentClosedWithoutConfirm, setPaymentClosedWithoutConfirm] =
    useState(false);
  ...
  const getDeliveryQuotes = async () => {
    ...
    try {
      ...
      if (!SAIL_API_KEY) {
        alert(
          "Sail API key not configured. Please set VITE_SAIL_API_KEY in your .env file"
        );
      ...
      if (!destinationCoords) {
        alert(
          'Could not find location. Please enter a valid address. You can also try "current location" to use your GPS location.'
        );
        setIsLoadingQuotes(false);
        return;
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(
          error.error?.message || "Failed to get delivery quotes"
        );
      }
  ...
  const geocodeAddress = async (address) => {
    try {
      ...
      if (
        address.toLowerCase().includes("current location") ||
        address.toLowerCase().includes("my location")
      ) {
  ...
  const handleProceedToPayment = () => {
    ...
    if (!phoneRegex.test(cleanedPhone)) {
      alert(
        "Please enter a valid Ghana phone number (e.g., 050xxxxxxx, +233xxxxxxxxx, or 233xxxxxxxxx)"
      );
      return;
    }
    ...
    if (!PAYSTACK_PUBLIC_KEY) {
      ...
      console.error(
        "Paystack public key is missing. Please set VITE_PAYSTACK_PUBLIC_KEY in your .env file"
      );
      return;
    }
    ...
    if (
      ...
      alert(
        "Invalid Paystack API key format. Please check your configuration."
      );
      console.error(
  ...
  return (
    <div className="relative w-full min-h-screen py-16 sm:py-20 overflow-x-hidden">
      <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 w-full">
          ...
          {orderType === "pickup" && (
            <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-black/10">
              <div className="flex items-start gap-3">
                <MapPin
                  className="text-[#ff9500] flex-shrink-0 mt-0.5"
                  size={20}
                />
              ...
              <div className="relative">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <button
                    ...
                    disabled={
                      isLoadingQuotes || !deliveryAddress.address.trim()
                    }
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex-1 min-w-0 pr-3">
                            ...
                            <div className="text-xs text-gray-500 mt-1 flex items-center gap-1 flex-wrap">
                              {quote.provider &&
                                quote.provider !== quote.service_name && (
                                  <>
                ...
                <div className="border-t border-gray-200 pt-3 mt-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xl sm:text-2xl font-bold text-[#ff9500] whitespace-nowrap">
                      ...
                      {(
                        getTotalPrice() +
                        (selectedQuote ? selectedQuote.price.amount : 0)
                      ).toFixed(2)}
                    </span>
                  </div>
                </div>
        ...
        {isProcessing && !paymentConfirming && (
          ...
          >
            ...
            <p className="mt-1 text-amber-700">
              If you're using Mobile Money, confirmation usually takes{" "}
              <strong>10–30 seconds</strong>. Please keep this page open and
              complete the prompt on your phone. Do not close the payment window
        ...
        {paymentConfirming && (
          <div
            className="mb-4 p-4 rounded-lg bg-green-50 border border-green-200 text-green-800 text-sm flex items-center gap-3"
            role="status"
          >
            ...
            <div>
              ...
              <p className="mt-1 text-green-700">
                Completing your order and redirecting…
              </p>
            </div>
          </div>
        )}
        {paymentClosedWithoutConfirm && (
          <div
            className="mb-4 p-4 rounded-lg bg-blue-50 border border-blue-200 text-blue-800 text-sm"
            role="alert"
          >
            <p className="font-medium">Payment window was closed</p>
            <p className="mt-1 text-blue-700">
              If you already completed the payment on your phone, it may still
              be processing. Wait a minute and refresh this page, or try
              "Proceed to Payment" again. If the amount was deducted but you
              don't see an order confirmation, contact us with your phone number
              and we'll help.
            </p>
            <button
              type="button"
